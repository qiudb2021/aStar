...
var TreeNode = /** @class */ (function () {
    ...
    TreeNode.prototype.view = function (center) {
        ...
        if (center) {
            ...
            var leftUp = util_1.convert(this._pos.x, this._pos.y);
            view_1.drawRectangle(leftUp, macro_1.GRID_WIDTH, macro_1.GRID_HEIGHT, macro_1.COLORS.green);
            var size = 10;
            leftUp.y += size;
            view_1.drawText(leftUp, "g: " + this._g + ";   h:" + this._h, macro_1.COLORS.yellow, size);
            leftUp.y += size;
            view_1.drawText(leftUp, "f: " + this.f, macro_1.COLORS.yellow, size);
            leftUp.y += size;
            view_1.drawText(leftUp, "(x:" + this.x + ",y:" + this.y + ")", macro_1.COLORS.black, size);
            leftUp.y += size * 2;
            view_1.drawText(leftUp, "index:" + this._index, macro_1.COLORS.black, size);
        }
    };
    TreeNode.index = 0;
    TreeNode._freeList = [];
